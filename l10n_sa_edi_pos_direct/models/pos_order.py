...
class PosOrder(models.Model):
    ...
    # UUID field for ZATCA - required for invoice identification
    # Note: In Odoo 17, uuid is on pos.order.line, not pos.order
    uuid = fields.Char(string='ZATCA UUID', readonly=True, copy=False, 
                       help="Unique identifier for ZATCA invoice tracking")
    ...
    l10n_sa_zatca_response = fields.Text(
        string="ZATCA Response",
        copy=False,
        help="Full response from ZATCA API including reportingStatus and validationResults"
    )
    ...
    def create(self, vals_list):
        ...
        for order in orders:
            if order._should_process_zatca():
                # Generate UUID for ZATCA if not already set
                if not order.uuid:
    ...
    def _is_refund_order(self):
        """Check if this is a refund order (has refunded_order_ids or negative amount)"""
        # Odoo 17: refunded_order_ids is Many2many, not Many2one
        return bool(self.refunded_order_ids) or self.amount_total < 0
    ...
    def _get_zatca_billing_reference_vals(self):
        ...
        """
        # Odoo 17: refunded_order_ids is Many2many, get first refunded order
        if self._is_refund_order() and self.refunded_order_ids:
            refunded_order = self.refunded_order_ids[0]  # Get first refunded order
            return {
                'id': refunded_order.pos_reference or refunded_order.name,
                'issue_date': refunded_order.date_order.strftime('%Y-%m-%d'),
            }
        return None
    ...
    def action_submit_to_zatca(self):
        """Manual action to submit order to ZATCA - for 'generated' or 'queued' status"""
        for order in self:
            if order.l10n_sa_zatca_status in ['generated', 'queued']:
                try:
                    order.submit_to_zatca_reporting()
                    _logger.info(f"ZATCA: Manual submission successful for order {order.name}")
                except Exception as e:
                    _logger.error(f"ZATCA: Manual submission failed for order {order.name}: {e}")
                    raise UserError(_("Failed to submit to ZATCA for order %s: %s") % (order.name, str(e)))
    ...
    def _generate_base64_qr_code(self):
        """Generate ZATCA-compliant QR code data for simplified invoices (Odoo 17 compatible)"""
        try:
            ...
            try:
                # Get the ZATCA journal and certificate (Odoo 17 approach)
                journal = self.session_id.config_id.invoice_journal_id
                if journal and journal._l10n_sa_ready_to_submit_einvoices():
                    # Odoo 17: Extract certificate from JSON field
                    import json
                    pcsid_json = journal.sudo().l10n_sa_production_csid_json
                    if pcsid_json:
                        pcsid_data = json.loads(pcsid_json)
                        x509_cert_b64 = pcsid_data.get('binarySecurityToken', '')
                        ...
                        if x509_cert_b64:
                            ...
                            b64_decoded_cert = b64decode(x509_cert_b64)
                            ...
                            public_key_str = base64.b64encode(public_key_bytes).decode('utf-8')[:20]
    ...
    def _submit_to_zatca_reporting_api(self, journal, xml_content):
        """Submit simplified invoice to ZATCA Reporting API (Odoo 17 compatible)"""
        try:
            # Odoo 17: _l10n_sa_api_get_pcsid returns dict directly
            PCSID_data = journal._l10n_sa_api_get_pcsid()
            
            # Extract certificate from PCSID data
            ...
            signed_xml = edi_format._l10n_sa_sign_xml(xml_content, x509_cert, digital_signature)
                        
            # Generate QR code from the signed XML
            # Odoo 17: _l10n_sa_get_qr_code expects pre-decoded certificate (single b64decode)
            from base64 import b64decode as _b64decode
            x509_cert_decoded = _b64decode(x509_cert).decode()
            qr_code_str = self.env['account.move']._l10n_sa_get_qr_code(
                journal, signed_xml, x509_cert_decoded, digital_signature, is_b2c=True
            )
            ...
            # Extract the invoice hash from the signed XML
            signed_xml_tree = etree.fromstring(final_xml if qr_nodes else signed_xml)
            ...
            if invoice_hash_nodes:
                ...
                temp_root = etree.fromstring(signed_xml)
                etree.indent(temp_root, space='    ')
                temp_xml = etree.tostring(temp_root)
            ...
            # Store ZATCA response for verification
            if result:
                self.l10n_sa_zatca_response = json.dumps(result, indent=2, ensure_ascii=False)
    ...
    def batch_submit_pending_zatca(self):
        ...
        # First get SA companies (country_id is non-stored, can't be searched directly)
        sa_companies = self.env['res.company'].search([]).filtered(
            lambda c: c.country_id.code == 'SA'
        )
        
        # Include both 'generated' and 'queued' statuses for submission
        pending_orders = self.search([
            ('l10n_sa_zatca_status', 'in', ['generated', 'queued']),
            ('company_id', 'in', sa_companies.ids),
            ('uuid', '!=', False),
        ])
